...
2. **Run setup**:
   ...
   ```
   This generates `app/app.yaml` and `databricks.yml` from templates, deploys the bundle, generates data, grants permissions, sets up vector search and IPS documents, and runs diagnostics.
...
3. **Access**: Open your Databricks workspace > **Apps** > `dev-investment-intel`.
...
dim_funds (fund investment metadata)
    |
    +-- fact_fund_performance (monthly returns and NAV)
    |
    +-- fact_portfolio_holdings (position-level holdings by type)
    |
    +-- fact_fund_flows (capital flow events by type)
    |
    +-- fact_portfolio_kpis (daily KPIs per manager/strategy)

portfolio_overview (VIEW - derived from dim_funds + performance)
```
...
| `SCHEMA` | Schema containing tables | `investment_intel` |
